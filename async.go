...
Depends on SDK types: *Omkar, types.IngestEventRequest, Events.IngestEvent.
*/
...
// AsyncClient provides an enhanced asynchronous client for the Omkar API
...
func (c *Omkar) NewAsyncClient() *AsyncClient {
...
func (c *Omkar) NewAsyncClientWithConfig(config AsyncConfig) *AsyncClient {
	...
	ac := &AsyncClient{
		...
		debugf: func(format string, args ...interface{}) {
			if config.Debug {
				fmt.Printf("[Omkar Debug] "+format+"\n", args...)
			}
		},
		errorCallback: func(err error) {
			if config.Debug {
				fmt.Printf("[Omkar Error] %v\n", err)
			}
		},
	}

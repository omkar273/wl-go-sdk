...
Type-safe Go client for the Omkar API: billing, metering, and subscription management for SaaS and usage-based products.
...
```bash
go get github.com/omkar273/wl-go-sdk/v2
```

Then in your code:

```go
import "github.com/omkar273/wl-go-sdk/v2"
```
...
| -------- | -------- | ----------- |
| `OMKAR_API_KEY` | Yes | API key |
| `OMKAR_API_HOST` | Optional | Full base URL including `https://` and `/v1` (default: `https://api.flexprice.io`). No trailing slash. |
...
import (
	...
	"github.com/omkar273/wl-go-sdk/v2"
...
func main() {
	...
	apiKey := os.Getenv("OMKAR_API_KEY")
	serverURL := os.Getenv("OMKAR_API_HOST")
	if serverURL == "" {
		serverURL = "https://api.flexprice.io"
	}
	if apiKey == "" {
		log.Fatal("Set OMKAR_API_KEY in .env or environment")
	}
...
func main() {
	...
	if apiKey == "" {
		log.Fatal("Set OMKAR_API_KEY in your environment")
	}
...
import (
	...
	"github.com/omkar273/wl-go-sdk/v2/models/dtos"
	"github.com/omkar273/wl-go-sdk/v2/models/types"
)
...
import (
	...
	"github.com/omkar273/wl-go-sdk/v2"
	"github.com/omkar273/wl-go-sdk/v2/errorutils"
	sdkerrors "github.com/omkar273/wl-go-sdk/v2/models/errors"
	"github.com/omkar273/wl-go-sdk/v2/models/types"
)
...
- Set the API key via the `x-api-key` header. Initialize with `flexprice.New(flexprice.WithServerURL(serverURL), flexprice.WithSecurity(apiKey))`, where `serverURL` is a full URL (default `https://api.flexprice.io`) or use `WithServerIndex` / default server list if you omit `WithServerURL`.
- Prefer environment variables; get keys from your [Omkar dashboard](https://app.flexprice.io) or docs.
...
- **Missing or invalid API key:** Ensure `OMKAR_API_KEY` is set and the key is active. Keys are usually server-side only; do not expose them in client-side code.
- **Wrong base URL:** Use a full URL such as `https://api.flexprice.io` (include `/v1`; no trailing slash). For local dev use `http://localhost:8080/v1` if applicable.
...
Omkar sends webhook events to your server for async updates on payments, invoices, subscriptions, wallets, and more.

**Flow:**
1. Register your endpoint URL in the Omkar dashboard
...
**Production rules:**
- Keep handlers idempotent — Omkar retries on non-`2xx`
- Return `200` for unknown event types — prevents unnecessary retries
- Do heavy processing async — respond fast, queue the work

## Documentation

- [Omkar API documentation](https://docs.flexprice.io)
- [Go SDK examples](examples/) in this repo
- [SDK integration tests](../tests/README.md) — full API coverage (different `OMKAR_API_HOST` shape; see that README)

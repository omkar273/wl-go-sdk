...
```go
omkarsdk.WithServerURL("https://api.example.com")
```
...
```go
omkarsdk.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
omkarsdk.WithClient(httpClient)
```
...
```go
omkarsdk.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
omkarsdk.WithTimeout(30 * time.Second)
```
...
```go
dtos.WithServerURL("http://api.example.com")
```
...
```go
dtos.WithTemplatedServerURL("http://{host}:{port}", map[string]string{
    "host": "api.example.com",
    "port": "8080",
})
```
...
```go
dtos.WithOperationTimeout(30 * time.Second)
```
...
```go
dtos.WithSetHeaders(map[string]string{
    "X-Cache-TTL": "60",
})
```
...
```go
dtos.WithURLOverride("/custom/path")
```
...
```go
dtos.WithAcceptHeaderOverride(dtos.AcceptHeaderEnumApplicationJson)
```